...
class RewardsBombs():
    def __init__(self):
        ...
        self.player_speed = 5
        self.score = 0
        self.font = pygame.font.SysFont("Cascadia Code",34)
        self.run()
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LEFT:
                        if self.green_pos[0] - self.player_speed >= 0:
                            self.green_pos[0] -= self.player_speed
                    elif event.key == pygame.K_RIGHT:
                        if self.green_pos[0] + self.player_speed <= self.screen_width:
                            self.green_pos[0] += self.player_speed
                    elif event.key == pygame.K_UP:
                         self.red_speed += 1
                         self.player_speed += 6
                    elif event.key == pygame.K_DOWN and self.player_speed !=1:
                        self.red_speed -= 1
                        self.player_speed -=1
            ...
            for pos in self.red_positions:
                if pos[2]:
                    if abs(pos[0] - self.green_pos[0]) <= 20 and abs(pos[1] - self.green_pos[1]) <= 20:
                        self.score += 1+self.player_speed
                        self.red_speed +=1
    ...
    def win(self):
        score_surface = self.font.render(f"Призы: {self.score}", True, (255, 255, 255))
        self.screen.blit(score_surface, (10, 10))
        message_surface = self.font.render(f"Вы собрали все награды! Призы: {self.score}", True, (255, 0, 0))
        ...
        exit()
    def pause(self):
        message_surface = self.font.render(f"Пауза", True, (255, 0, 0))
        self.screen.blit(message_surface, (self.screen_width // 2 - message_surface.get_width() // 2, self.screen_height // 2 - message_surface.get_height() // 2))
        pygame.display.update()
